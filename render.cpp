...
void render_t::init ( ) {
    ...
    /* init font map */
    fonts [ fonts_t::hack_watermark ] = font_data_t ( XOR ( "Verdana" ), 12, 400, 0, font_flags_t::font_flag_anti_aliasing | font_flags_t::font_flag_drop_shadow );
...
bool render_t::world_to_screen ( const vec_t &origin, vec_t &screen ) {
    ...
    static auto view_matrix = pattern::find( XOR ( "client.dll" ), XOR ( "0F 10 05 ? ? ? ? 8D 85 ? ? ? ? B9" ) ).add ( 3 ).deref ( ).add ( 176 ).as< std::uintptr_t > ( );
