...
void inputpred_t::predict ( ucmd_t *ucmd ) {
	...
	if ( !m_prediction_player || !m_prediction_seed ) {
		m_prediction_seed = pattern::find ( x_ ( "client.dll" ), x_ ( "8B 0D ? ? ? ? BA ? ? ? ? E8 ? ? ? ? 50" ) ).add ( 0x2 ).deref ( ).as< uintptr_t > ( );
		m_prediction_player = pattern::find ( x_ ( "client.dll" ), x_ ( "83 3D ? ? ? ? ? 74 ? 51 8B 0D" ) ).add ( 0x2 ).deref ( ).as< uintptr_t > ( );
	}
